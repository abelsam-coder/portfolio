...
const Testimony = () => {
  ...
  const StarIcon = ({ filled }) => (
    <svg className={`w-5 h-5 ${filled ? 'text-yellow-400' : 'text-slate-300 dark:text-slate-600'}`} fill="currentColor" viewBox="0 0 20 20">
      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
    </svg>
  );

  const QuoteIcon = () => (
    <svg className={`w-10 h-10 opacity-20 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    ...
    `}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`
          relative max-w-4xl mx-auto mb-16 p-8 md:p-12 rounded-3xl backdrop-blur-xl border overflow-hidden
          ${isDarkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-gradient-to-br from-slate-50 to-blue-50/30 border-slate-200'}
        `}>
          ...
          {testimonials.map((testimonial, index) => (
            <div
              ...
              className={`
                ...
                ${activeIndex === index
                  ? isDarkMode
                    ? 'bg-slate-900/90 border-blue-500/50 shadow-xl shadow-blue-500/10 scale-[1.02]'
            ...
            >
              ...
              {/* Content Preview */}
              <p className={`
                text-sm leading-relaxed line-clamp-4 mb-4
                ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}
              `}>
                "{testimonial.content}"
              </p>
              ...
              `}>
                <span className={`
                  ...
                  bg-gradient-to-r ${testimonial.color} bg-opacity-10 text-transparent bg-clip-text
                `}
                style={{ background: 'none', color: isDarkMode ? '#93c5fd' : '#2563eb' }}
                >
                  📌 {testimonial.project}
                </span>
                
                <svg className={`w-5 h-5 transition-transform group-hover:translate-x-1 ${
                  activeIndex === index ? (isDarkMode ? 'text-blue-400' : 'text-blue-600') : (isDarkMode ? 'text-slate-600' : 'text-slate-400')
        ...
        `}>
          <p className={`text-sm font-medium mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
